...
<div class="bg-primary-light rounded-lg shadow p-6 border border-primary-accent">
    <h3 class="text-lg font-semibold text-primary-dark mb-4">Add New Relationship</h3>
    
    <form action="{{ route('relationships.store', $card) }}" method="POST" hx-boost="true">
        @csrf
        
        <div class="mb-4">
            <label for="related_card_id" class="block text-sm font-medium text-primary-dark mb-2">Card *</label>
            <input 
                type="hidden" 
            ...
            />
            <div id="autocomplete_results" class="mt-2 border border-primary-accent rounded-lg hidden max-h-48 overflow-y-auto bg-primary-light"></div>
        ...
        <div class="mb-4">
            <label for="relationship_type" class="block text-sm font-medium text-primary-dark mb-2">Relationship Type *</label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', async function() {
        ...
        try {
            ...
            if (cards.length === 0) {
                resultsDiv.innerHTML = '<div class="p-2 text-primary-danger">No results found</div>';
            ...
            resultsDiv.innerHTML = cards.map(card => `
                <div 
                    class="p-2 cursor-pointer hover:bg-primary-light border-b last:border-b-0"
