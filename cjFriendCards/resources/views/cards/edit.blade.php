...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Edit Friendship Card</h1>
</div>
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('cards.update', $card) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="birthday" class="block text-sm font-medium text-gray-700 mb-2">Birthday (dd-mm-yyyy)</label>
                    ...
                    <p class="text-gray-500 text-xs mt-1">Enter date as dd-mm-yyyy (e.g., 25-12-1990)</p>
                    @error('birthday')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="4" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ old('notes', $card->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-4">
                    <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Update Card</button>
                    <a href="{{ route('cards.show', $card) }}" class="bg-gray-600 text-white px-6 py-2 rounded hover:bg-gray-700">Cancel</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div>
        <div class="bg-white rounded-lg shadow p-6 sticky top-8">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    <p class="text-gray-500">Unique Name</p>
                    <p class="text-gray-900 font-medium">{{ $card->unique_name }}</p>
                </div>
                <div>
                    <p class="text-gray-500">Full Name</p>
                    <p class="text-gray-900 font-medium">{{ $card->full_name }}</p>
                </div>
                @if ($card->birthday)
                    <div>
                        <p class="text-gray-500">Current Age</p>
                        <p class="text-gray-900 font-medium">{{ (int) abs(now()->diffInYears($card->birthday)) }} years</p>
                    </div>
                @endif
                <div class="pt-4 border-t mt-4">
                    <p class="text-gray-500 text-xs">Created: {{ $card->created_at->format('M d, Y') }}</p>
                    <p class="text-gray-500 text-xs">Updated: {{ $card->updated_at->format('M d, Y') }}</p>
                </div>
            </div>
        </div>
    </div>
